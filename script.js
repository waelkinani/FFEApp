...
async function doSubmit() {
  if (!APPS_SCRIPT_URL || APPS_SCRIPT_URL === 'YOUR_APPS_SCRIPT_URL_HERE') {
    alert('Setup required:\n\nPlease deploy Code.gs as a Google Apps Script Web App and paste the URL into the APPS_SCRIPT_URL constant in script.js.\n\nSee README.md for step-by-step instructions.');
    return;
  }
  ...
  try {
    ...
    document.getElementById('success-detail').textContent =
      `${rows.length} rows sent to "${SHEET_NAME}" › ${SHEET_TAB}`;
    fullReset();

  } catch (e) {
    btn.disabled  = false;
    btn.innerHTML = '<i class="ti ti-send" style="font-size:13px" aria-hidden="true"></i> Submit';
    alert(`Submission failed: ${e.message}\n\nCheck that your Apps Script Web App URL is correct and deployed as "Anyone" access.`);
  }
}
